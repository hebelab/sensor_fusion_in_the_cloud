...
\begin{abstract}
Depth perception in autonomous systems is critically enhanced through sensor fusion, where the integration of stereo vision and LiDAR data plays a pivotal role. However, effectively merging sparse LiDAR measurements with dense stereo imagery remains a key challenge. This paper introduces a novel fusion method based on the Papoulis-Gerchberg algorithm, which iteratively refines stereo depth maps using LiDAR data as anchor points. By leveraging these anchor points, our approach enhances depth map resolution and accuracy, particularly in areas where traditional methods struggle, such as object boundaries and edge definition.

We validate our method using the KITTI benchmark suite, demonstrating substantial improvements over existing approaches like FastFusion, particularly in edge sharpness and object boundary clarity. These results highlight the potential of our method to not only achieve superior fidelity in depth estimation but also to set a new standard for sensor data integration in real-time autonomous navigation.

To test our approach in real-world conditions, we introduce a versatile mobile sensor suite, comprising a Velodyne VLP16 LiDAR, ZED Stereo Camera, and an NVIDIA Jetson Nano for on-board processing. This sensor suite is mounted on a legged robot platform to simulate vibrations and disturbances common in military and civilian applications. The collected data is processed using the ROS framework, and SLAM algorithms are employed to evaluate the system’s performance, providing a complete test loop for autonomous navigation tasks.
\end{abstract}
